...
//	ia=*(int*)0xC300;
	...
	int f1 = -1;
	while (f1 < 0){f1 = FIO_CreateFile("A:/STDOUT.TXT"); SleepTask(100);}
	ioGlobalStdSet(1,f1);

	int f2 = -1;
	while (f2 < 0){f2 = FIO_CreateFile("A:/STDERR.TXT"); SleepTask(100);}
	ioGlobalStdSet(2,f2);
	...
	{	//ChangeDprData(41,1); //this proc enable iso 16-80, 2000-3200
		...
		sprintf(message,"%u %02X\n\0", pMessage[0], pMessage[1]);
		FIO_WriteFile(file, message, strlen(message));
...
void CreateMyTask()
{
	hMyTaskMessQue=(int*)CreateMessageQueue("MyTaskMessQue",0xFF);
...
//*/
	...
	for(i = 0; i < ptr[0]; i++)
		SendMyMessage(id, ptr[i]);

	if(ptr[1] == BUTTON_DP)
		SendMyMessage(0, GUIMode);
